...
Begin VB.Form Form_Data_DJG 
   ...
   End
End
Attribute VB_Name = "Form_Data_DJG"
...
Private Sub chapus_Click()
    ...
    If x = vbYes Then
        ...
        strsql = "Select id,no_part,proses,jic,size,type,size_2,metal,radius,width,partition,partition2,filler,marking_stamp_lokal_or " & _
            "from code where deleted=0 and category=2"
...
Private Sub cmbfiller_KeyPress(KeyAscii As Integer)
...
Public Sub DrawListview()
    With ListView1
        ...
        .ColumnHeaders.Add 13, , "Filler", 1000
        .ColumnHeaders.Add 14, , "Stamp Marking", 3000
    End With
End Sub
...
Public Sub FillListview(strsql As String)
    ...
    If Not rscode.EOF Then
        ...
        Do While Not rscode.EOF
            ...
            lst.SubItems(12) = IIf(IsNull(rscode!filler), "", rscode!filler)
            lst.SubItems(13) = rscode!marking_stamp_lokal_or
            rscode.MoveNext
        Loop
    End If
End Sub
...
Public Sub EditData(strcode As String)
    If rscode.State = 1 Then rscode.Close
    strsql = "Select id,no_part,proses,jic,size,type,size_2,metal,radius,width,partition,partition2,filler,marking_stamp_lokal_or " & _
        "from code where deleted=0 and category=2 and id='" & strcode & "'"
    rscode.Open strsql, conn, adOpenDynamic, adLockOptimistic
    If Not rscode.EOF Then
...
Public Sub save()
    If rscode.State = 1 Then rscode.Close
    strsql = "Select no_part from code where no_part='" & txtnopart.Text & "' "
    rscode.Open strsql, conn, adOpenDynamic, adLockOptimistic
    ...
    If Not rscode.EOF Then
        ...
        txtnopart.SetFocus
    Else
        strsql = "insert into code(no_part,proses,jic,size,type,size_2,metal,radius,width,partition,partition2,filler,marking_stamp_lokal_or,deleted,category) " & _
            "values ('" & txtnopart & "','" & cmbproses & "','" & txtjic & "','" & txtsize & "','" & cmbtype & "','" & tsize & "','" & tmetal & "' " & _
            ",'" & txtradius & "','" & txtwidth & "','" & txtpartitionbar & "','" & txtpartitionbar2 & "','" & cmbfiller & "','" & tmarking_or_lokal & "',0,2) "
        conn.Execute (strsql)
...
Public Sub update()
    strsql = "update code set proses='" & cmbproses & "',jic='" & txtjic & "', size='" & txtsize & "', " & _
        ...
        "width='" & txtwidth & "',partition='" & txtpartitionbar & "',partition2='" & txtpartitionbar2 & "', filler='" & cmbfiller & "', " & _
...
Public Sub search(strcari As String)
    strsql = "Select id,no_part,proses,jic,size,type,size_2,metal,radius,width,partition,partition2,filler,marking_stamp_lokal_or " & _
        "from code where deleted=0 and category=2 and (no_part like '%" & strcari & "%' OR proses like '%" & strcari & "%' " & _
        ...
        "or width like '%" & strcari & "%' or partition like '%" & strcari & "%' or partition2 like '%" & strcari & "%' or filler like '%" & strcari & "%' " & _
