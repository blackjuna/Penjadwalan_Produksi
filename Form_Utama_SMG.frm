...
Begin VB.Form Form_Utama_SMG 
   ...
   Begin VB.PictureBox Picture1 
      Height          =   1020
      Left            =   13680
      Picture         =   "Form_Utama_SMG.frx":0000
      ScaleHeight     =   960
      ScaleWidth      =   975
      TabIndex        =   64
      Top             =   7080
   ...
   Begin VB.Frame Frame4 
      ...
      Begin VB.ComboBox cproses2 
         Height          =   315
         ItemData        =   "Form_Utama_SMG.frx":0B9D
         Left            =   1440
         List            =   "Form_Utama_SMG.frx":0B9F
         TabIndex        =   49
         Top             =   720
         Width           =   2655
      End
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.ComboBox cshift 
         Height          =   315
         ItemData        =   "Form_Utama_SMG.frx":0BA1
         Left            =   1800
         List            =   "Form_Utama_SMG.frx":0BAB
         TabIndex        =   21
         Top             =   1680
         Width           =   735
      End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cstatus 
         Height          =   315
         ItemData        =   "Form_Utama_SMG.frx":0BB5
         Left            =   1800
         List            =   "Form_Utama_SMG.frx":0BBC
         TabIndex        =   7
         Top             =   3240
         Width           =   2655
      End
   ...
   Begin VB.Label T_Page 
      ...
      Index           =   0
      Left            =   1200
      MouseIcon       =   "Form_Utama_SMG.frx":0BCA
   ...
   End
End
Attribute VB_Name = "Form_Utama_SMG"
...
'    kapasitas = rsdata_mesin!MyCapa
    ...
    strsql = "select sum(qty) AS MyPending from completion_slip where " & _
        ...
        "finish_date='" & Format(ListView1.SelectedItem.SubItems(6), "yyyy/mm/dd") & "' and status='Reschedule' and category=3"
    ...
    qty_last_slip = "SELECT qty From completion_slip WHERE finish_date = '" & tanggal2 & "' " & _
        ...
        "AND finish_date='" & tanggal2 & "' and shift='1' and category=3)"
    ...
    Do While pending > 0
        ...
        For s = 1 To 2
            qty_nextdate_1 = "select sum(qty) AS MyTotal from completion_slip where " & _
                "proses_2='" & ListView1.SelectedItem.SubItems(11) & "' and " & _
                "finish_date='" & Format(DateAdd("d", 1, tanggal), "YYYY/mm/dd") & "' and shift='" & s & "' and category=3"
            ...
            Else
                ...
                Do Until pindahqty >= sisapending
                    qty_last_slip = "SELECT qty From completion_slip WHERE " & _
                        ...
                        "AND finish_date='" & Format(DateAdd("d", 1, tanggal), "YYYY/mm/dd") & "' and shift='" & s & "' and category=3)"
                    If rscompletion_slip.State = 1 Then rscompletion_slip.Close
                    rscompletion_slip.Open qty_last_slip, conn, adOpenDynamic, adLockOptimistic
                    If Not rscompletion_slip.EOF Then
...
Public Sub code_part()
    ...
    strsql = "select no_part from code where deleted=0 and category=3"
...
On Error GoTo Err_1
    ...
    rscompletion_slip.Open "select no_slip, no_so, jic, Size, finish_date, qty, delivery_date, status, " & _
        "qty_pending,remarks_produksi,proses_2,Type,id from completion_slip where category=3 and deleted =0 order by id desc", conn, adOpenDynamic, adLockOptimistic
...
Public Sub Create_No_Slip()
    ...
    rscompletion_slip.Open "select date_printed,lot_number,batch_number,no_slip from completion_slip where deleted =0 and " & _
        "category=3 order by id asc", conn, adOpenDynamic, adLockOptimistic
...
'            batch_number = qry_batch_number
    ...
    If rscompletion_slip.RecordCount = 0 Then
        Me.tno_slip.Text = "2017-0001-S0000001"
    Else
        rscompletion_slip.MoveLast
        Z = Val(Mid(rscompletion_slip!no_slip, 12, 10)) + 1
'        Y = Mid(rscompletion_slip!no_slip, 12, 10)
        If Z < 10 Then
            Me.tno_slip.Text = thn & "-" & tbatchnumber.Text & "-" & "S000000" & Z
        ElseIf Z < 100 Then
            Me.tno_slip.Text = thn & "-" & tbatchnumber.Text & "-" & "S00000" & Z
        ElseIf Z < 1000 Then
            Me.tno_slip.Text = thn & "-" & tbatchnumber.Text & "-" & "S0000" & Z
        ElseIf Z < 10000 Then
            Me.tno_slip.Text = thn & "-" & tbatchnumber.Text & "-" & "S000" & Z
        ElseIf Z < 100000 Then
            Me.tno_slip.Text = thn & "-" & tbatchnumber.Text & "-" & "S00" & Z
        ElseIf Z < 1000000 Then
            Me.tno_slip.Text = thn & "-" & tbatchnumber.Text & "-" & "S0" & Z
        ElseIf Z < 10000000 Then
            Me.tno_slip.Text = thn & "-" & tbatchnumber.Text & "-" & "S" & Z
        End If
    End If
End Sub
...
Public Sub SearchProduct()
    ...
    strsql = "select jic,proses,size,type,filler,size_2,filler,metal,marking_stamp_lokal_or,radius,width,partition,partition2 from code where " & _
        "no_part='" & cno_part.Text & "' and deleted=0 and category=3"
    rscode.Open strsql, conn, adOpenDynamic, adLockOptimistic
    If Not rscode.EOF Then
        ...
        txt_metal.Text = IIf(IsNull(rscode.Fields("metal")), "", rscode.Fields("metal"))
        tsize2.Text = rscode.Fields("size_2")
        tmetal.Text = IIf(IsNull(rscode.Fields("metal")), "", rscode.Fields("metal"))
...
Public Sub available_qty()
    strsql = "select sum(qty+qty_pending) AS MyTotal from completion_slip where left(type,1)='" & Left(ttype, 1) & "' " & _
        "and finish_date='" & Format(Me.dtfinish_date.Value, "YYYY/mm/dd") & "' and shift='" & Me.cshift.Text & "' and category=3"
    Set rscompletion_slip = conn.Execute(strsql)
    
    If cnowinding.Text = "-" Then
        lblavaqty.Caption = "Unlimited"
    Else
        available = IIf(Left(ttype, 1) = "A", 10, 10) - Val(IIf(IsNull(rscompletion_slip.Fields("MyTotal")), "0", rscompletion_slip.Fields("MyTotal")))
...
Public Sub save()
    ...
    strsql = "Select no_slip from completion_slip where " & _
        "no_slip='" & tno_slip.Text & "' and deleted=0 and category=3"
    rscode.Open strsql, conn, adOpenDynamic, adLockOptimistic
    If Not rscode.EOF Then
    ...
    Else
        ...
        "lot_number,batch_number,proses_1,proses_2,proses_3,no_part,shift,jic,size,type," & _
        ...
        "'" & tsize.Text & "','" & ttype.Text & "'," & _
        "'" & tmarking_or.Text & "','" & tqty.Text & "','" & tcustomer.Text & "','" & cstatus.Text & "'," & _
        "'" & tnote.Text & "',0,3,0,'" & txt_Metal_cert.Text & "','" & txt_filler_cert.Text & "')"
        conn.Execute (strsql)
...
Public Sub SetEdit()
    ...
    Me.txt_metal.Text = IIf(IsNull(rscompletion_slip.Fields("metal")), "", rscompletion_slip.Fields("metal"))
